...
class TenantMigrateShardCommand extends Command
{
    protected $signature = 'tenant:migrate-shard
        {database : Workspace shard database name e.g. hrms_company_b_workspace}
        {--create : Create database if missing (requires MySQL CREATE privilege)}';

    protected $description = 'Run workspace migrations on a tenant shard database (Company B provisioning)';
    ...
    public function handle(): int
    {
        ...
        $this->info("Running workspace migrations on [{$database}]...");
