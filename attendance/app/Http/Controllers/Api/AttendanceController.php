...
class AttendanceController extends Controller
{
    ...
    public function monthlySummary(Request $request): JsonResponse
    {
        try {
            ...
            // Get week-off configurations for the employee
            ...
            if (empty($weekOffDays)) {
                $weekOffDays = [0]; // Sunday by default
            }
            ...
            while ($currentDate->lte($endDate)) {
                ...
                if ($record) {
                    $existingIn = $record->check_in_time;
                    $existingOut = $record->check_out_time;
                    $effectiveStatus = $record->status;
                    
                    if ($raw) {
                        if (!$existingIn && $raw->check_in_time) {
                            $existingIn = $raw->check_in_time;
                        }
                        if (!$existingOut && $raw->check_out_time) {
                            $existingOut = $raw->check_out_time;
                        }
                        if (in_array($effectiveStatus, ['future', 'absent', 'pm']) && $raw->status) {
                            $hasMissingCheckout = ($raw->check_in_time && !$raw->check_out_time && $isPast);
                    ...
                    if (($existingIn && !$existingOut) || (!$existingIn && $existingOut)) {
                        if ($existingIn && $isToday) {
                    ...
                    if (($raw->check_in_time && !$raw->check_out_time) || (!$raw->check_in_time && $raw->check_out_time)) {
                        if ($raw->check_in_time && $isToday) {
                            $effectiveStatus = 'present';
                        } else {
                            $effectiveStatus = 'pm';
                        }
                ...
                if ($record || $raw || $holiday || $leave || $isFuture) {
                    $attendanceData = [
                        'status' => $effectiveStatus,
                        'check_in_time' => $existingIn,
                        'check_out_time' => $existingOut,
                        'total_hours' => $record ? $record->total_hours : ($raw ? $raw->total_hours : null),
                    ];
                }
            ...
            $isLocked = $attendanceRecords->where('is_locked', true)->isNotEmpty();
    ...
    public function storeCorrection(Request $request): JsonResponse
    {
        try {
            ...
            if ($isLocked) {
                return response()->json([
                    'success' => false,
                    'message' => 'Attendance for this month is locked. You cannot submit correction requests.'
                ], 400);
            }
