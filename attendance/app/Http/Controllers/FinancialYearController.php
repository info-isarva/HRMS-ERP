...
class FinancialYearController extends Controller
{
    ...
    {
        // Sync financial years from Payroll API
        try {
            $payrollApiService = app(\App\Services\PayrollApiService::class);
            $payrollFys = $payrollApiService->getFinancialYears();
            
            if ($payrollFys) {
                foreach ($payrollFys as $payrollFy) {
                    ...
                    FinancialYear::updateOrCreate(
                        ['name' => $normalizedName],
                        [
                            'start_date' => \Carbon\Carbon::parse($payrollFy['start_date'])->tz('Asia/Kolkata')->toDateString(),
                            'end_date' => \Carbon\Carbon::parse($payrollFy['end_date'])->tz('Asia/Kolkata')->toDateString(),
                            'is_active' => $payrollFy['is_current'] ?? false,
                            'status' => ($payrollFy['is_closed'] ?? false) ? 'closed' : 'open',
                        ]
                    );
                }
            }
        } catch (\Exception $e) {
            \Illuminate\Support\Facades\Log::error('Failed to sync financial years from payroll API: ' . $e->getMessage());
        }
