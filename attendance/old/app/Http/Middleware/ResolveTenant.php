...
class ResolveTenant
{
    ...
    {
        $this->tenantContext->setModule((string) config('tenant.module', 'attendance'));
        ...
        if (config('tenant.resolve_from_session')) {
            $tenant = $this->tenantLogin->resolveFromSession();
            $source = 'session';
        }
    ...
    private function continueWithTenant(Request $request, Closure $next, Tenant $tenant, string $source): Response
    {
        ...
        try {
            $this->databaseManager->switchForTenant($tenant, $this->tenantContext->module() ?? 'attendance');
        } catch (\Throwable $e) {
