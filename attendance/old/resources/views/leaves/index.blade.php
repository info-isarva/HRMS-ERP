            ...
            <div class="bg-gradient-to-r from-blue-600 to-indigo-700 px-8 py-10">
                <div class="flex items-center justify-between">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold text-white">{{ $isSelfView ? 'My Leaves' : 'Pending Leaves' }}</h1>
                            <p class="text-blue-100 text-xs sm:text-sm lg:text-base mt-2">
                                @if($isSelfView)
                                    Manage your own leave requests and track their status
                                @else
                                    Review and manage pending employee leave applications
                                @endif
                            </p>
                        </div>
                    </div>
        ...
        <div class="bg-white rounded-2xl shadow border border-gray-100 overflow-hidden">
            <div class="px-4 sm:px-6 py-3 sm:py-4 border-b bg-gradient-to-r from-gray-50 to-gray-100">
                <h3 class="text-lg sm:text-xl font-bold text-gray-800 flex items-center">
                    ...
                    {{ $isSelfView ? 'My Leave Applications' : 'Pending Leaves' }}
                </h3>
            </div>
            @if($leaves->count() > 0)
                <!-- Desktop Table -->
                                ...
                                <tr class="hover:bg-indigo-50 transition leave-row" 
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-2">
                                            ...
                                            {{-- HR/Admin actions --}}
                                            @if(!$isSelfView && (auth()->user()->isAdmin() || auth()->user()->isSuperAdmin() || auth()->user()->role === 'hr'))
