...
@section('title', 'Add Public Holiday - HRMS')
...
<div class="container mx-auto max-w-full px-4 py-4 sm:py-6">
    ...
    <div class="mb-6 sm:mb-8">
        <div class="flex flex-col sm:flex-row items-center justify-between bg-gradient-to-r from-indigo-600 to-blue-600 px-4 py-6 sm:px-8 sm:py-10 rounded-lg shadow-lg">
            <div class="flex items-center space-x-3 sm:space-x-4 mb-4 sm:mb-0">
                ...
                <div>
                    <h1 class="text-2xl sm:text-3xl lg:text-4xl font-bold text-white">Add Public Holiday</h1>
                    <p class="text-blue-100 mt-1 sm:mt-2 text-sm sm:text-base">Create a new public holiday for your organization</p>
                </div>
            </div>
    ...
    <!-- Form -->
    <form method="POST" action="{{ route('public-holidays.store') }}" class="bg-white shadow-sm rounded-lg overflow-hidden">
        @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                    ...
                    <div>
                        ...
                        <select id="financial_year" name="financial_year" 
                                ...
                                required onchange="updateDepartmentConfigs()">
                            @foreach($financialYears as $key => $year)
                                <option value="{{ $key }}" {{ old('financial_year', $selectedYear) == $key ? 'selected' : '' }}>
                                    {{ $year }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select id="type" name="type" 
                                class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('type') border-red-500 @enderror"
                                required>
                        ...
                        <p class="mt-1 text-xs text-gray-500">
                            ...
                            <strong>Note:</strong> Fixed holidays are mandatory for all employees in departments that have this holiday assigned. Flexible holidays provide options - employees can choose which flexible holidays to take based on their department's flexible holiday quota (e.g., if assigned 5 flexible holidays, employees can choose any 5 from all available flexible holidays). Holiday type cannot be changed after creation.
                        </p>
                        @error('type')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                            ...
                            <div id="departmentGrid" class="p-3 sm:p-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2 sm:gap-3">
                                @foreach($departments as $department)
                                    @php
                                        ...
                                        // Default availability (will be updated by JavaScript)
                                        $isAvailable = $hasConfig;
                                    @endphp
                                    <div class="department-item relative {{ !$hasConfig ? 'opacity-50' : '' }}" 
                                         data-department-name="{{ strtolower($department->name) }}"
                                         data-department-code="{{ strtolower($department->code) }}"
                                         data-available="{{ $hasConfig ? 'true' : 'false' }}"
                                            ...
                                            <div class="flex items-start space-x-2">
                                                ...
                                                <div class="flex-1 min-w-0">
                                                    ...
                                                    <div class="flex items-center justify-between mt-2">
                                                        ...
                                                        @if($hasConfig)
                                                            <span class="quota-info inline-flex items-center px-1.5 py-0.5 sm:px-2 sm:py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-700">
                                                                <i class="fas fa-check text-xs mr-1"></i>
                                                                <span class="quota-text">{{ $config->remaining_holidays }}</span>
                                                            </span>
                                                        @else
                        ...
                        <div class="mt-3 p-3 sm:p-4 bg-blue-50 border border-blue-200 rounded-lg">
                            <div class="flex items-start">
                                ...
                                <div class="text-sm text-blue-800">
                                    <p class="font-medium mb-1">Department Selection Guidelines:</p>
                                    <ul class="list-disc pl-4 space-y-1 text-xs">
                                        <li><strong>Fixed Holidays:</strong> Limited by the fixed holiday quota set for each department. These are mandatory holidays that all employees in the department must take.</li>
                                        <li><strong>Flexible Holidays:</strong> No creation limits apply. Employees can choose which flexible holidays to take based on their department's flexible holiday quota (e.g., choose any 5 from 10 available flexible holidays).</li>
                                        <li><strong>No Config:</strong> Departments without holiday configuration cannot be selected.</li>
                                        <li>The quota display shows: <span class="font-mono">remaining/total</span> for fixed holidays, <span class="font-mono">Unlimited</span> for flexible holidays.</li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                    ...
                    <div>
                        ...
                        <div class="flex items-center space-x-3">
                            <input type="color" id="color" name="color" value="{{ old('color', '#3b82f6') }}" 
                                   ...
                                   required>
                            <input type="text" id="color-hex" value="{{ old('color', '#3b82f6') }}" 
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        typeSelect.addEventListener('change', function() {
            updateDepartmentAvailability(this.value);
        });
        
        // Initialize with current type
        updateDepartmentAvailability(typeSelect.value);
    });
    
    function updateDepartmentAvailability(selectedType) {
        const departmentItems = document.querySelectorAll('.department-item');
        
        departmentItems.forEach(item => {
            const hasConfig = item.dataset.hasConfig === 'true';
            const remainingFixed = parseInt(item.dataset.remainingFixed || 0);
            const remainingFlexible = item.dataset.remainingFlexible;
            const usedFixed = parseInt(item.dataset.usedFixed || 0);
            const usedFlexible = parseInt(item.dataset.usedFlexible || 0);
            const totalFixed = parseInt(item.dataset.totalFixed || 0);
            const totalFlexible = parseInt(item.dataset.totalFlexible || 0);
            
            const checkbox = item.querySelector('.dept-checkbox');
            const quotaInfo = item.querySelector('.quota-info');
            const quotaText = quotaInfo ? quotaInfo.querySelector('.quota-text') : null;
            const quotaIcon = quotaInfo ? quotaInfo.querySelector('i') : null;
            
            let isAvailable = hasConfig;
            let displayText = '';
            let statusClass = '';
            
            if (!hasConfig) {
                isAvailable = false;
                displayText = 'No Config';
                statusClass = 'bg-red-100 text-red-700';
            } else if (selectedType === 'fixed') {
                isAvailable = remainingFixed > 0;
                displayText = `${remainingFixed}/${totalFixed}`;
                statusClass = isAvailable ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700';
            } else if (selectedType === 'flexible') {
                isAvailable = true; // Always available for flexible
                displayText = 'Unlimited';
                statusClass = 'bg-blue-100 text-blue-700';
            } else {
                // No type selected yet
                isAvailable = hasConfig;
                displayText = hasConfig ? 'Available' : 'No Config';
                statusClass = hasConfig ? 'bg-gray-100 text-gray-700' : 'bg-red-100 text-red-700';
            }
            ...
            // Update visual state
            item.classList.toggle('opacity-50', !isAvailable);
            item.dataset.available = isAvailable ? 'true' : 'false';
            
            // Update quota display
            if (quotaInfo && quotaText) {
                quotaText.textContent = displayText;
                quotaInfo.className = `quota-info inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${statusClass}`;
                
                if (quotaIcon) {
                    quotaIcon.className = `fas ${isAvailable ? 'fa-check' : 'fa-times'} text-xs mr-1`;
                }
            }
        });
    ...
    function updateDepartmentConfigs() {
        const selectedYear = document.getElementById('financial_year').value;
        
        // Show loading state
        const departmentGrid = document.getElementById('departmentGrid');
        departmentGrid.innerHTML = '<div class="col-span-full flex items-center justify-center py-8"><i class="fas fa-spinner fa-spin text-blue-600 mr-2"></i>Loading departments...</div>';
        
        // Redirect to refresh with new year
        const url = new URL(window.location);
        url.searchParams.set('financial_year', selectedYear);
