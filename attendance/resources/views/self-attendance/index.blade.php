                        ...
                        @for($day = 1; $day <= $daysInMonth; $day++)
                            @php
                                ...
                                if ($record) {
                                    ...
                                    // Fallback to raw attendance times if not present in the record
                                    if ($rawAttendance) {
                                        if (!$existingIn && $rawAttendance->check_in_time) {
                                            $existingIn = \Carbon\Carbon::parse($rawAttendance->check_in_time)->format('H:i');
                                        }
                                        if (!$existingOut && $rawAttendance->check_out_time) {
                                            $existingOut = \Carbon\Carbon::parse($rawAttendance->check_out_time)->format('H:i');
                                        }
                                        if (in_array($effectiveStatus, ['future', 'absent', 'pm']) && $rawAttendance->status) {
                                            // Only override if it's not a past date with missing checkout
                                            $hasMissingCheckout = ($rawAttendance->check_in_time && !$rawAttendance->check_out_time && $isPast);
