...
class EmployeeController extends Controller
{
     ...
     */
    private function getDepartmentsFromMaster($locationId = 0)
    {
        $query = Department::where('status', 1);
        
        // Always include location 0 (All) and the specific location if provided
        $locationId = ($locationId === null || $locationId === '') ? 0 : $locationId;
        $query->whereIn('location_id', [0, $locationId]);
     ...
     */
    private function getDesignationsFromMaster($locationId = 0)
    {
        $query = PositionType::where('status', 1);
     ...
     */
    private function getDocumentTypesFromMaster($locationId = 0)
    {
        $query = DocumentType::where('status', 1);
    ...
    /**
     * Calculate monthly TDS based on annual salary and regime slabs
     */
    private function calculateTdsValue($annualCtc, $regime)
    {
        if (empty($annualCtc) || $annualCtc <= 0) {
            return 0;
        }

        $groupSettings = \App\Models\Setting::where('group', 'salary')->pluck('value', 'key');
        $tdsEnabled = ($groupSettings['salary_tds_enabled'] ?? '') == '1';
        if (!$tdsEnabled) {
            return 0;
        }

        $slabsKey = $regime === 'old' ? 'salary_tds_slabs_old' : 'salary_tds_slabs_new';
        $slabs = json_decode($groupSettings[$slabsKey] ?? '[]', true);
        ...
        foreach ($slabs as $slab) {
            $from = floatval($slab['from'] ?? 0);
            $to = isset($slab['to']) && $slab['to'] !== '' ? floatval($slab['to']) : INF;
            $percentage = floatval($slab['percentage'] ?? 0);

            if ($taxableIncome > $from) {
                $taxableRange = min($taxableIncome, $to) - $from;
                $taxForSlab = ($taxableRange * $percentage) / 100;
                $totalTax += $taxForSlab;
            }
        }
    ...
    public function getMastersByLocation(Request $request)
    {
        $locationId = $request->location_id;
        
        $statutoryComponents = StatutoryComponent::where('status', '1')->where(function($q) use ($locationId) {
            $q->whereJsonContains('location_id', 0)
              ->orWhereJsonContains('location_id', '0');
            if ($locationId) {
                $q->orWhereJsonContains('location_id', (int)$locationId)
                  ->orWhereJsonContains('location_id', (string)$locationId);
            }
        })->get();

        $salaryComponents = SalaryComponent::where('status', '1')->where(function($q) use ($locationId) {
            $q->whereJsonContains('location_id', 0)
              ->orWhereJsonContains('location_id', '0');
            if ($locationId) {
                $q->orWhereJsonContains('location_id', (int)$locationId)
        ...
        return response()->json([
            'success' => true,
            'departments' => $this->getDepartmentsFromMaster($locationId),
            'designations' => $this->getDesignationsFromMaster($locationId),
            'documentTypes' => $this->getDocumentTypesFromMaster($locationId),
            'statutoryComponentIds' => $statutoryComponents->pluck('id'),
            'salaryComponentIds' => $salaryComponents->pluck('id'),
        ]);
    }
    ...
    public function createEmployee()
    {
        ...
        return view('employees.form', [
            ...
            'designations' => $this->getDesignationsFromMaster(0),
            'departments' => $this->getDepartmentsFromMaster(0),
            'statuses' => $this->getEmployeeStatusesFromMaster(),
            'roles' => $this->getRolesFromMaster(),
            'transaction_types' => getTransactionTypes(),
            'bloodGroups' => getBloodGroups(),
            'paymentTypes' => getPaymentTypes(),
            'statutoryComponents' => StatutoryComponent::where('status', '1')->get(),
            ...
            'documentTypes' => $this->getDocumentTypesFromMaster(0),
            'locations' => $this->getLocationsFromMaster(),
            'settings' => $settings,
        ]);
    ...
    public function saveEmployee(Request $request)
    {
        ...
        try {
            ...
            $basicDataToSave['enable_crm'] = $request->has('basic.enable_crm') ? 1 : 0;
            ...
            $request->validate([
                ...
                'statutory_components.*.tds_regime' => 'nullable|in:new,old',
                'statutory_components.*.tds_option' => 'nullable|in:auto,manual_value',
            ]);
            ...
            foreach ($statutoryComponents as $component) {
                if (
                    ...
                    $val = isset($component['value']) && is_numeric($component['value']) ? $component['value'] : 0;
                    if ($component['statutory_component_id'] == 3 && ($component['tds_option'] ?? '') === 'auto') {
                        $val = $this->calculateTdsValue($basic->annual_ctc ?? 0, $component['tds_regime'] ?? 'new');
                    }
                    ...
                    $basic->statutoryComponents()->create([
                        ...
                        'tds_regime' => $component['tds_regime'] ?? null,
                        'tds_option' => $component['tds_option'] ?? null,
                        'created_by' => auth()->id(),
                        'updated_by' => auth()->id()
                    ]);
                }
            ...
            if (!empty($basic->email) && $basic->enable_self_portal) {
                ...
                // Sync newly created user with CRM system if CRM is enabled
                if ($basic->enable_crm) {
                    $this->syncNewUserWithCRM($user, $defaultPassword);
                }
            }
                ...
                $createdEmployeeData = [
                    ...
                    'statutory_components' => $basic->statutoryComponents->map(function($comp) {
                        return [
                            ...
                            'tds_regime' => $comp->tds_regime,
                            'tds_option' => $comp->tds_option,
                        ];
                    })->toArray(),
    ...
    public function editEmployee(EmployeeBasicDetail $employee)
    {
        ...
        return view('employees.form', [
            ...
            'genders' => getGenders(),
            'maritalStatuses' => getMaritalStatuses(),
            'designations' => $this->getDesignationsFromMaster($employee->location_id),
            'departments' => $this->getDepartmentsFromMaster($employee->location_id),
            'statuses' => $this->getEmployeeStatusesFromMaster(),
            'roles' => $this->getRolesFromMaster(),
            'transaction_types' => getTransactionTypes(),
            'bloodGroups' => getBloodGroups(),
            ...
            'enableSelfPortalMasterSetting' => $enableSelfPortalMasterSetting,
            'documentTypes' => $this->getDocumentTypesFromMaster($employee->location_id),
            'locations' => $this->getLocationsFromMaster(),
            'settings' => $settings,
        ]);
    }
    ...
    public function updateEmployee(Request $request, EmployeeBasicDetail $employee)
    {
        ...
        try {
            ...
            // Handle enable_self_portal - maintain current value if checkbox not present (disabled state)
            // or update based on checkbox if present
            if ($request->has('basic.enable_self_portal_present')) {
                $basicDataToUpdate['enable_self_portal'] = $request->has('basic.enable_self_portal') ? 1 : 0;
            }

            // Handle enable_crm
            if ($request->has('basic.enable_crm_present')) {
                $basicDataToUpdate['enable_crm'] = $request->has('basic.enable_crm') ? 1 : 0;
            }
            
            // Handle enable_payroll from basic array
            // Only update if the checkbox was actually present in the form (submitted via permissions tab)
            if ($request->has('basic.enable_payroll_present')) {
                $basicDataToUpdate['enable_payroll'] = $request->has('basic.enable_payroll') ? 1 : 0;
            }
            
            // Apply master setting default for enable_self_portal ONLY if the form field wasn't present at all
            if (!$request->has('basic.enable_self_portal_present')) {
                $enableSelfPortalMasterSetting = \App\Models\Setting::getValue('enable_self_portal', false);
                if ($enableSelfPortalMasterSetting) {
            ...
            foreach ($newStatutoryComponents as $component) {
                if (
                    ...
                    $val = isset($component['value']) && is_numeric($component['value']) ? $component['value'] : 0;
                    if ($component['statutory_component_id'] == 3 && ($component['tds_option'] ?? '') === 'auto') {
                        $val = $this->calculateTdsValue($employee->annual_ctc ?? 0, $component['tds_regime'] ?? 'new');
                    }
            ...
            if ($employee->enable_self_portal) {
                if ($user) {
                    ...
                    $updateData = [
                        ...
                        'enable_self_portal' => $employee->enable_self_portal,
                        'enable_payroll' => $employee->enable_payroll,
                        'enable_crm' => $employee->enable_crm,
                    ];
                    ...
                    // Sync with CRM if CRM is enabled
                    if ($employee->enable_crm) {
                        $this->syncNewUserWithCRM($user, '');
                    }
                ...
                // Self portal is disabled — update user with actual employee values
                // Only set user inactive if ALL access flags are disabled
                $updateData = [
                    'enable_self_portal' => 0,
                    'enable_payroll' => $employee->enable_payroll,
                    'enable_crm' => $employee->enable_crm,
                ];
                
                // Determine user status: only deactivate if no access flags are enabled
                if (!$employee->enable_payroll && !$employee->enable_crm) {
                    $updateData['status'] = 'Inactive';
                    $message = 'Employee updated successfully. User account has been deactivated.';
                } else {
                    // Keep user active since they still have payroll or CRM access
                    $userStatus = 'Active';
                    if ($employee->status) {
                        $employeeStatus = \App\Models\EmployeeStatus::find($employee->status);
                        if ($employeeStatus) {
                    ...
                    $updateData['avatar'] = $employee->profile_image ?? $user->avatar;
                    if (!empty($employee->email)) {
                        $updateData['email'] = $employee->email;
                    }
                    $message = 'Employee updated successfully.';
                ...
                // Sync with CRM if CRM is enabled (even when self portal is off)
                if ($employee->enable_crm) {
                    $this->syncNewUserWithCRM($user, '');
                }
            } else {
 ...
 /**
    ...
    public function offerLetterPDF(EmployeeBasicDetail $employee, PDFGenerator $pdfGenerator)
    {
        ...
        $netPayAnualy = $netPay * 12;
        $inWords = NumberHelper::numberToWords($netPayAnualy);
 ...
 /**
    ...
    /**
     * Sync newly created user from employee conversion with CRM system
     *
     * @param \App\Models\User $user
     * @param string $plainPassword
     * @return void
     */
    private function syncNewUserWithCRM($user, $plainPassword)
    {
        try {
            $syncEnabled = env('CRM_SYNC_ENABLED', true);
            if (!$syncEnabled) {
                \Log::info("CRM sync is disabled", ['user_id' => $user->user_id]);
                return;
            }

            $crmApiUrl = env('CRM_API_BASE_URL');
            $apiToken = env('CRM_API_TOKEN', 'hrms_sync_token_2025_secure_key');

            if (empty($crmApiUrl) || empty($apiToken)) {
                \Log::warning("CRM API configuration missing", [
                    'url' => $crmApiUrl,
            ...
            // Get employee record for additional data
            $employee = \App\Models\EmployeeBasicDetail::find($user->employee_id);
            ...
            $fullUrl = rtrim($crmApiUrl, '/') . '/users/sync-simple';

            \Log::info("Attempting to sync user with CRM", [
                'url' => $fullUrl,
                'user_id' => $user->user_id,
                'data' => $syncData
            ]);
            ...
            ->timeout(10)
            ->connectTimeout(5)
            ->post($fullUrl, $syncData);

            if ($response->successful()) {
                \Log::info("User synced successfully with CRM", [
                    'user_id' => $user->user_id,
                    'response' => $response->json()
                ]);

                // Sync password with CRM
                $this->syncPasswordWithCRM($user->user_id, $plainPassword);
            } else {
                \Log::warning("Failed to sync user with CRM", [
                    'user_id' => $user->user_id,
            ...
            \Log::error("Error syncing user with CRM", [
                'user_id' => $user->user_id ?? 'unknown',
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
            // Don't throw — avoid breaking employee creation
        }
    }

    /**
     * Sync password with CRM system
     *
     * @param string $userId Employee ID string
     * @param string $plainPassword
     * @return bool
     */
    private function syncPasswordWithCRM($userId, $plainPassword)
    {
        try {
            $crmApiUrl = env('CRM_API_BASE_URL');
            $apiToken = env('CRM_API_TOKEN', 'hrms_sync_token_2025_secure_key');
            ...
            $fullUrl = rtrim($crmApiUrl, '/') . '/users/sync-password';
            ...
            if ($response->successful()) {
                \Log::info("CRM password synced successfully", ['user_id' => $userId]);
                return true;
            } else {
                \Log::warning("Failed to sync password with CRM", [
                    'user_id' => $userId,
                    'status' => $response->status(),
                    'response' => $response->body()
                ]);
            ...
            \Log::error("Error syncing password with CRM", [
                'user_id' => $userId,
                'error' => $e->getMessage()
            ]);
            return false;
