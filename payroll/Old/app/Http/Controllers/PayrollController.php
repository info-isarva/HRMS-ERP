...
class PayrollController extends Controller
{
    ...
    public function attendance($month, $year)
    {
        ...
        // Fetch employees: Active OR resigned/last working day in the same month
            ...
            ->where(function ($query) use ($startOfMonth) {
                $query->whereNull('date_of_resignation') // Still working
                    ->orWhere(function($q) use ($startOfMonth) {
                        $q->whereNotNull('date_of_resignation')
                          ->where(function($sub) use ($startOfMonth) {
                              $sub->whereDoesntHave('exitDetails')
                                  ->whereDate('date_of_resignation', '>=', $startOfMonth);
                          })
                          ->orWhereHas('exitDetails', function($sub) use ($startOfMonth) {
                              $sub->whereNotIn('status', ['Approved', 'Completed'])
                                  ->orWhereDate('last_working_day', '>=', $startOfMonth);
                          });
                    });
            })
