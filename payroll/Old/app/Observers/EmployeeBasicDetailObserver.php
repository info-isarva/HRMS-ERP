...
class EmployeeBasicDetailObserver
{
    ...
    public function created(EmployeeBasicDetail $employee)
    {
        ...
        // $this->syncToTimeStation($employee);
    }

    /**
     * Sync new employee to TimeStation
     */
    protected function syncToTimeStation(EmployeeBasicDetail $employee)
    {
        // Resolve Department Name
        $departmentName = \DB::table('departments')->where('id', $employee->department)->value('department') ?? 'General';
        
        // Resolve Designation (Title)
        $designationName = \DB::table('position_types')->where('id', $employee->designation)->value('position') ?? 'Employee';
        
        // Generate PIN (Last 4 digits of Employee ID or default)
        $pin = '0000';
        if (!empty($employee->employee_id)) {
            // Remove non-digits
            $digits = preg_replace('/\D/', '', $employee->employee_id);
            if (strlen($digits) >= 4) {
                $pin = substr($digits, -4);
            } else {
                $pin = str_pad($digits, 4, '0', STR_PAD_LEFT);
            }
        }
        ...
        $data = [
            ...
            'custom_employee_id' => $employee->employee_id,
            'title' => $designationName,
            'email' => $employee->email,
            'hourly_rate' => $employee->ot_per_hour ?? '0.00',
            'pin' => $pin,
            'phone' => $employee->contact_number,
            'start_date' => $employee->date_of_joining,
        ];
        
        $tsService = app(\App\Services\TimeStationService::class);
        $tsService->createEmployee($data);
    }
