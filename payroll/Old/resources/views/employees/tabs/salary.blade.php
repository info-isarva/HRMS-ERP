         ...
         <div class="row g-4 align-items-center">
            <div class="col-md-3">
                ...
                <div class="input-group shadow-sm rounded-3 bg-white">
                    <span class="input-group-text bg-transparent border-0 text-muted ps-3">{{ get_currency_symbol() }}</span>
                        ...
                        <table class="table table-modern mb-0">
                            <thead>
                                <tr>
                                    ...
                                    <th class="col-input text-end">Monthly Amount ({{ get_currency_symbol() }})</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($salaryComponents->where('status', '1') as $index => $component)
                                    ...
                                    @endphp
                                    <tr class="salary-component-row" data-component-id="{{ $component->id }}" data-index="{{ $index }}">
                                        ...
                                        <td class="align-middle">
                                            <div id="salary_row_{{ $index }}" style="{{ $isChecked ? '' : 'display: block;' }}">
                                                ...
                                                <div class="input-group input-group-sm">
                                                    <span class="input-group-text bg-transparent border-0 text-muted px-1">{{ get_currency_symbol() }}</span>
                                                    <input type="number" step="0.01"
                    ...
                    <div class="table-responsive">
                        <table class="table table-modern mb-0">
                            ...
                            @foreach($statutoryComponents->where('status', '1') as $index => $component)
                                ...
                                <tr class="statutory-component-row {{ $isPF ? 'bg-light' : '' }}" data-component-id="{{ $component->id }}" data-index="{{ $index }}">
                                    <td class="text-center align-middle">
                                        <div class="form-check form-switch d-flex justify-content-center">
                                            <input type="checkbox"
                                    ...
                                    <td class="align-middle">
                                        ...
                                        {{-- Nested TDS Controls --}}
                                        @if(strtolower($component->short_name) === 'tds')
                                            @php
                                            ...
                                            @endphp
                                            <div id="statutory_row_options_tds_{{ $index }}" style="{{ $isChecked ? '' : 'display: block;' }}" class="mt-2 text-muted">
                                                <div class="d-flex align-items-center gap-2 flex-wrap">
                                                    <select name="statutory_components[{{ $index }}][tds_regime]"
                                                            class="form-select form-select-sm tds-regime border-0 bg-white shadow-sm"
                                                    ...
                                                    <select name="statutory_components[{{ $index }}][tds_option]"
                                                            class="form-select form-select-sm tds-option border-0 bg-white shadow-sm"
                                                            id="tds_option_{{ $index }}"
                                                            onchange="window.toggleTDSValueField({{ $index }})"
                                                            style="width: auto; font-size: 0.8rem;">
                                                        <option value="auto" {{ $tdsOption == 'auto' ? 'selected' : '' }}>Auto Calc</option>
                                                        <option value="manual_value" {{ $tdsOption == 'manual_value' ? 'selected' : '' }}>Manual</option>
                                                    </select>
                                                </div>
                                            </div>
                                        @endif
                                        ...
                                        <div id="statutory_row_{{ $index }}" style="{{ $isChecked ? '' : 'display: block;' }}">
                                            ...
                                            <div class="input-group input-group-sm">
                                                ...
                                                <input type="number" step="0.01"
                                                       ...
                                                       value="{{ $value }}"
                                                       id="{{ $isPF ? 'epf_value_'.$index : (strtolower($component->short_name) === 'tds' ? 'tds_value_'.$index : '') }}"
                                                       {{ (strtolower($component->short_name) === 'tds' && $tdsOption == 'auto') ? 'readonly' : '' }}
                                                       data-type="{{ $component->type }}"
                                                       data-short-name="{{ strtolower($component->short_name) }}"
                                                       data-calc-type="{{ $component->calculation_type ?? 'flat_amount' }}"
                                                       data-calc-value="{{ $component->calculation_value ?? 0 }}"
    ...
    if (typeof window.toggleEPFValueField === 'undefined') {
        window.toggleEPFValueField = function(index) {
             ...
             if(epfOption.value === 'manual_value') {
                 label.innerHTML = 'Manual Value (' + window.globalCurrencySymbol + ')';
                 valueInput.placeholder = 'Enter Amount';
             } else {
                 label.innerHTML = 'Value (' + window.globalCurrencySymbol + ')';
    ...
    window.calculateSalaryStructure = function() {
                ...
                components.forEach(c => {
                    ...
                    if(c.type === 'earning') {
                        ...
                        if(s.includes('pf') || s.includes('epf')) {
                            ...
                            const indexMatch = c.input.id.match(/tds_value_(\d+)/);
                            if (indexMatch) {
                                const idx = indexMatch[1];
                                const tdsOptionSelect = document.getElementById('tds_option_' + idx);
                                const tdsRegimeSelect = document.getElementById('tds_regime_' + idx);
                                
                                if (tdsOptionSelect && tdsOptionSelect.value === 'auto') {
                                    if (!window.tdsEnabled) {
                                        val = 0;
                                    } else {
                                        const regime = tdsRegimeSelect ? tdsRegimeSelect.value : 'new';
                                        ...
                                        let totalTax = 0;
                                        const annualCtcVal = parseFloat(document.getElementById('annual_ctc').value) || 0;
                                        let taxableIncome = annualCtcVal;
        ...
        if (typeof window.toggleTDSValueField === 'undefined') {
            window.toggleTDSValueField = function(index) {
                ...
                if (tdsOption && tdsOption.value === 'manual_value') {
                    if(valueLabel) valueLabel.innerHTML = 'Manual TDS Amount ({{ get_currency_symbol() }}) <span class="text-danger">*</span>';
                    if(valueInput) {
                        valueInput.placeholder = 'Enter manual TDS amount';
                        valueInput.removeAttribute('readonly');
                    }
                } else {
                    if(valueLabel) valueLabel.innerHTML = 'Calculated Value ({{ get_currency_symbol() }})';
                    if(valueInput) {
        ...
        // Initialize TDS fields on page load
        ...
        tdsSelects.forEach(function(select) {
            ...
            if (indexMatch) {
                window.toggleTDSValueField(indexMatch[1]);
            }
        });
