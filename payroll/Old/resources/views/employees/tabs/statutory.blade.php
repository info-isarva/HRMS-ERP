        ...
        <div class="card-body">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <div class="input-group">
                        <span class="input-group-text text-success fw-bold">{{ get_currency_symbol() }}</span>
        ...
        <div class="card mb-3 shadow-sm statutory-card">
            <div class="card-body">
                ...
                <div id="statutory_row_{{ $index }}" class="row g-3 align-items-center" 
                        ...
                        <div class="col-md-3">
                            ...
                            <select name="statutory_components[{{ $index }}][epf_option]"
                                ...
                                <option value="restrict_15000" {{ $epfOption == 'restrict_15000' ? 'selected' : '' }}>
                                    Restrict to {{ get_currency_symbol() }}15,000 (Traditional)
                                </option>
                        ...
                        <div class="col-md-2">
                            <label class="form-label fw-semibold" id="epf_value_label_{{ $index }}">
                                @if($epfOption == 'manual_value')
                                    Manual EPF Amount ({{ get_currency_symbol() }}) <span class="text-danger" id="epf_value_required_{{ $index }}">*</span>
                                @else
                                    Reference Value ({{ get_currency_symbol() }})
                                @endif
                            </label>
                            <input type="number"
                    ...
                    @elseif(strtolower($component->short_name) === 'tds')
                        @php
                            $tdsRegime = old("statutory_components.{$index}.tds_regime", $employeeComponent->tds_regime ?? 'new');
                            $tdsOption = old("statutory_components.{$index}.tds_option", $employeeComponent->tds_option ?? 'auto');
                        @endphp
                        <div class="col-md-3">
                            <label class="form-label fw-semibold">Tax Regime <span class="text-danger">*</span></label>
                            <select name="statutory_components[{{ $index }}][tds_regime]"
                                    class="form-control tds-regime-select"
                                    id="tds_regime_{{ $index }}"
                                    onchange="window.calculateStatutoryComponents()">
                                <option value="new" {{ $tdsRegime == 'new' ? 'selected' : '' }}>New Regime</option>
                                <option value="old" {{ $tdsRegime == 'old' ? 'selected' : '' }}>Old Regime</option>
                            </select>
                        </div>

                        <div class="col-md-3">
                            <label class="form-label fw-semibold">Calculation Option <span class="text-danger">*</span></label>
                            <select name="statutory_components[{{ $index }}][tds_option]"
                                    class="form-control tds-option-select"
                                    id="tds_option_{{ $index }}"
                                    onchange="window.toggleTDSValueField({{ $index }})">
                                <option value="auto" {{ $tdsOption == 'auto' ? 'selected' : '' }}>Auto Calculate</option>
                                <option value="manual_value" {{ $tdsOption == 'manual_value' ? 'selected' : '' }}>Manual Value</option>
                            </select>
                        </div>

                        <div class="col-md-2">
                            <label class="form-label fw-semibold" id="tds_value_label_{{ $index }}">
                                @if($tdsOption == 'manual_value')
                                    Manual TDS Amount ({{ get_currency_symbol() }}) <span class="text-danger">*</span>
                                @else
                                    Calculated Value ({{ get_currency_symbol() }})
                                @endif
                            </label>
                            <input type="number" step="0.01"
                                   ...
                                   value="{{ $value }}"
                                   id="tds_value_{{ $index }}"
                                   placeholder="@if($tdsOption == 'manual_value') Enter manual TDS amount @else Calculated automatically @endif"
                                   {{ $tdsOption == 'auto' ? 'readonly' : '' }}>
                            @error("statutory_components.{$index}.value")
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    @else
                        <div class="col-md-8">
                            <label class="form-label fw-semibold">Value ({{ get_currency_symbol() }}) <span class="text-danger" id="statutory_value_required_{{ $index }}" style="{{ $isChecked ? '' : 'display: none;' }}">*</span></label>
                            <input type="number" step="0.01"
                                   name="statutory_components[{{ $index }}][value]"
                                   class="form-control component-value-statutory @error("statutory_components.{$index}.value") is-invalid @enderror"
                                   value="{{ $value }}"
...
<script>
    // Define TDS Slabs and settings from backend
    window.tdsSlabsNew = {!! json_encode(json_decode($settings['salary_tds_slabs_new'] ?? '[]', true)) !!};
    window.tdsSlabsOld = {!! json_encode(json_decode($settings['salary_tds_slabs_old'] ?? '[]', true)) !!};
    window.tdsEnabled = {{ ($settings['salary_tds_enabled'] ?? '0') == '1' ? 'true' : 'false' }};
    ...
    if (typeof window.toggleEPFValueField === 'undefined') {
        window.toggleEPFValueField = function(index) {
            ...
            if (epfOption.value === 'manual_value') {
                valueLabel.innerHTML = 'Manual EPF Amount ({{ get_currency_symbol() }}) <span class="text-danger" id="epf_value_required_' + index + '">*</span>';
                valueInput.placeholder = 'Enter manual EPF amount';
                valueInput.required = true;
                helpText.textContent = 'This amount will be deducted as EPF';
            } else {
                valueLabel.innerHTML = 'Reference Value ({{ get_currency_symbol() }})';
    ...
    if (typeof window.toggleTDSValueField === 'undefined') {
        window.toggleTDSValueField = function(index) {
            const tdsOption = document.getElementById('tds_option_' + index);
            const valueLabel = document.getElementById('tds_value_label_' + index);
            const valueInput = document.getElementById('tds_value_' + index);

            if (tdsOption.value === 'manual_value') {
                valueLabel.innerHTML = 'Manual TDS Amount ({{ get_currency_symbol() }}) <span class="text-danger">*</span>';
                valueInput.placeholder = 'Enter manual TDS amount';
                valueInput.required = true;
                valueInput.removeAttribute('readonly');
            } else {
                valueLabel.innerHTML = 'Calculated Value ({{ get_currency_symbol() }})';
                valueInput.placeholder = 'Calculated automatically';
                valueInput.required = false;
                valueInput.setAttribute('readonly', 'readonly');
            }
            window.calculateStatutoryComponents();
        };
    }
    ...
    if (typeof window.calculateStatutoryComponents === 'undefined') {
        window.calculateStatutoryComponents = function() {
            ...
            // Find Annual CTC
            const annualCtcInput = document.getElementById('annual_ctc');
            const annualCtc = parseFloat(annualCtcInput ? annualCtcInput.value : 0) || 0;
            ...
            statutoryInputs.forEach(input => {
                ...
                // Check if this is TDS
                if (input.id.startsWith('tds_value_')) {
                    const indexMatch = input.id.match(/tds_value_(\d+)/);
                    if (indexMatch) {
                        const idx = indexMatch[1];
                        const tdsOption = document.getElementById('tds_option_' + idx);
                        const tdsRegime = document.getElementById('tds_regime_' + idx);
                        
                        if (tdsOption && tdsOption.value === 'auto') {
                            if (!window.tdsEnabled) {
                                input.value = '0.00';
                            } else {
                                const regime = tdsRegime ? tdsRegime.value : 'new';
                                const slabs = regime === 'old' ? window.tdsSlabsOld : window.tdsSlabsNew;
                                
                                // Sort slabs copy by 'from' ascending
                                const slabsCopy = [...slabs];
                                slabsCopy.sort((a, b) => (parseFloat(a.from) || 0) - (parseFloat(b.from) || 0));
                                
                                let totalTax = 0;
                                let taxableIncome = annualCtc;
                                
                                slabsCopy.forEach(slab => {
                                    const from = parseFloat(slab.from) || 0;
                                    const to = slab.to && slab.to !== '' ? parseFloat(slab.to) : Infinity;
                                    const percentage = parseFloat(slab.percentage) || 0;
                                    
                                    if (taxableIncome > from) {
                                        const range = Math.min(taxableIncome, to) - from;
                                        totalTax += (range * percentage) / 100;
                                    }
                                });
                                
                                const monthlyTds = totalTax / 12;
                                input.value = monthlyTds.toFixed(2);
                            }
                        }
                    }
                    return;
    ...
    // Initialize EPF and TDS fields on page load
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Find all TDS option dropdowns and initialize them
        const tdsSelects = document.querySelectorAll('select[name*="tds_option"]');
        tdsSelects.forEach(function(select) {
            const indexMatch = select.id.match(/tds_option_(\d+)/);
            if (indexMatch) {
                toggleTDSValueField(indexMatch[1]);
            }
        });
