...
@section('title', 'Manage Employee Statuses')
@section('content')
<style>
    /* Page Header Card */
...
<div class="page-wrapper">
    <div class="content container-fluid">
        ...
        <div class="page-header-card">
                ...
                <div class="d-flex justify-content-between align-items-center">
                    <div class="d-flex align-items-center">
                        <div class="page-header-icon-box">
                            <i class="fas fa-user-tag fa-lg" style="color: rgba(255,255,255,0.9);"></i>
                        </div>
                        <div class="ms-3">
                            <h1 class="page-header-title">Employee Statuses</h1>
                            <p class="page-header-subtitle">Manage employee status types used across the system</p>
                        </div>
                    </div>
                    <!-- Stats Section -->
                    <div class="d-flex align-items-center text-end d-none d-md-flex">
                        <div class="page-header-stats me-3 text-white">
                            <p class="page-header-stats-label mb-0" style="opacity: 0.9; font-size: 0.875rem;">Total Statuses</p>
                            <p class="page-header-stats-value mb-0 fw-bold" style="font-size: 1.75rem;">{{ $employeeStatuses->count() }}</p>
                        </div>
            ...
            <div class="p-3 d-flex justify-content-between align-items-center flex-wrap gap-3">
                ...
                <div class="d-flex gap-2">
                    <a href="#" class="btn btn-primary" data-toggle="modal" data-target="#add_employee_status">
        ...
        <div class="table-card">
            <div class="table-card-header">
                <h4><i class="fas fa-list me-2"></i>Status List</h4>
            </div>
                    ...
                    <table class="table table-striped custom-table datatable">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $employeeStatus->status_name }}</td>
                                <td>
                                    <span class="badge badge-modern badge-{{ $employeeStatus->status ? 'success' : 'danger' }}">{{ $employeeStatus->status ? 'Active' : 'Inactive' }}</span>
                                </td>
                                <td class="text-end">
                                    <button class="btn-action btn-action-edit me-2 employeeStatusEdit" data-id="{{ $employeeStatus->id }}"><i class="fas fa-edit me-1"></i> Edit</button>
                                    <button class="btn-action btn-action-delete employeeStatusDelete" data-id="{{ $employeeStatus->id }}" data-toggle="modal" data-target="#delete_employee_status"><i class="fas fa-trash me-1"></i> Delete</button>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title"><i class="fas fa-plus-circle me-2"></i> Add Employee Status</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form action="{{ route('form/employee-status/save') }}" method="POST">
                        @csrf
                        @if ($errors->any())
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <h5 class="mb-3 text-uppercase bg-light p-2 rounded"><i class="fas fa-info-circle me-2 text-primary"></i>Employee Status Details</h5>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group mb-3">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="text" class="form-control @error('status_name') is-invalid @enderror" name="status_name" placeholder="Enter status name" value="{{ old('status_name') }}" required>
                                    </div>
                                    @error('status_name')
    ...
    <div id="edit_employee_status_modal" class="modal custom-modal fade" role="dialog">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title"><i class="fas fa-edit me-2"></i> Edit Employee Status</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span>&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form id="editForm" method="POST" action="{{ route('form/employee-status/update') }}">
                        @csrf
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="form-group mb-3">
                                    ...
                                    <div class="input-group">
                                        <span class="input-group-text"><i class="fas fa-user-tag"></i></span>
                                        <input type="text" class="form-control @error('status_name') is-invalid @enderror" name="status_name" id="edit_status_name" placeholder="Enter status name" value="{{ old('status_name') }}" required>
                                    </div>
                                    @error('status_name')
                                        <div class="invalid-feedback d-block">{{ $message }}</div>
                                    @enderror
    ...
    <!-- Delete Employee Status Modal -->
    <div id="delete_employee_status" class="modal custom-modal fade" role="dialog">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title"><i class="fas fa-trash-alt me-2"></i>Delete Employee Status</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span>&times;</span>
                    </button>
                </div>
                <div class="modal-body text-center">
                    ...
                    <p class="text-muted mb-4">This action cannot be undone. This will permanently delete the employee status.</p>
                    <div class="d-flex justify-content-center gap-3">
                        <form id="deleteForm" method="POST" action="{{ route('form/employee-status/delete') }}" class="d-inline">
                            @csrf
    ...
    <!-- /Delete Employee Status Modal --></div>
@section('script')
<script>
    const baseUrl = "{{ url('/') }}";
    $(document).ready(function () {
        $(document).on('click', '.employeeStatusEdit', function (e) {
            e.preventDefault();
            var id = $(this).data('id');

            $.ajax({
                url: `${baseUrl}/form/employee-status/get/${id}`,
                type: 'GET',
                success: function (response) {
                    ...
                    $('#edit_employee_status_modal').modal('show');
                },
                error: function (xhr, status, error) {
                    console.error('Error fetching employee status:', error);
                    alert('Could not fetch employee status data.');
                }
            });
        });

        $(document).on('click', '.employeeStatusDelete', function (e) {
            e.preventDefault();
            var id = $(this).data('id');
            $('#delete_id').val(id);
        });
        ...
        @if ($errors->any())
            @if (old('id'))
                ...
                setTimeout(function() {
                    $('#edit_id').val('{{ old('id') }}');
                    $('#edit_status_name').val('{{ old('status_name') }}');
                    $('#edit_status').val('{{ old('status') }}');
                    $('#edit_employee_status_modal').modal('show');
                }, 100);
            @else
                // Add operation failed - show add modal
                $('#add_employee_status').modal('show');
            @endif
        @endif
