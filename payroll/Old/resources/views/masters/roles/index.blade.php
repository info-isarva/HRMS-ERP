...
<div class="page-wrapper">
    <div class="content container-fluid">
        ...
        <div class="page-header-card">
                ...
                <div class="d-flex justify-content-between align-items-center">
                    <div class="d-flex align-items-center">
                        ...
                        <div class="ms-3">
                            ...
                            <p class="page-header-subtitle">Manage system roles and permissions</p>
                        </div>
                    </div>
                    <!-- Stats Section -->
                    <div class="d-flex align-items-center text-end d-none d-md-flex">
                        <div class="page-header-stats me-3 text-white">
                            <p class="page-header-stats-label mb-0" style="opacity: 0.9; font-size: 0.875rem;">Total Roles</p>
                            <p class="page-header-stats-value mb-0 fw-bold" style="font-size: 1.75rem;">{{ $roles->count() }}</p>
                        </div>
                        <div class="page-header-stats-icon p-2 rounded" style="background: rgba(255,255,255,0.2); width: auto; height: auto;">
                            <i class="fas fa-user-lock text-white" style="font-size: 2rem;"></i>
                        </div>
                    </div>
                </div>
            </div>
            <div class="p-3 d-flex justify-content-between align-items-center flex-wrap gap-3">
                ...
                <div class="d-flex gap-2">
                    <a href="#" class="btn btn-primary" data-toggle="modal" data-target="#add_role">
        ...
        <div class="table-card">
            <div class="table-card-header">
                <h4><i class="fas fa-list me-2"></i>Role List</h4>
            </div>
                    ...
                    <table class="table table-striped custom-table datatable">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="badge badge-modern badge-{{ $role->status ? 'success' : 'danger' }}">{{ $role->status ? 'Active' : 'Inactive' }}</span>
                                </td>
                                <td class="text-end">
                                    <button class="btn-action btn-action-edit me-2 roleEdit" data-id="{{ $role->id }}"><i class="fas fa-edit me-1"></i> Edit</button>
                                    <button class="btn-action btn-action-delete roleDelete" data-id="{{ $role->id }}" data-toggle="modal" data-target="#delete_role"><i class="fas fa-trash me-1"></i> Delete</button>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title"><i class="fas fa-plus-circle me-2"></i> Add Role</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                </div>
                <div class="modal-body">
                    <form action="{{ route('form/role/save') }}" method="POST">
                        @csrf
                        @if ($errors->any())
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <h5 class="mb-3 text-uppercase bg-light p-2 rounded"><i class="fas fa-info-circle me-2 text-primary"></i>Role Details</h5>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group mb-3">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="text" class="form-control @error('role_name') is-invalid @enderror" name="role_name" placeholder="Enter role name" value="{{ old('role_name') }}" required>
                                    </div>
                                    @error('role_name')
                        ...
                        <div class="submit-section"><button type="submit" class="btn btn-primary submit-btn">Submit</button></div>
                    </form>
                </div>
            </div>
        </div>
    ...
    <div id="edit_role_modal" class="modal custom-modal fade" role="dialog">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="editForm" method="POST" action="{{ route('form/role/update') }}">@csrf<input type="hidden" name="id" id="edit_id">@if ($errors->any())<div class="alert alert-danger"><ul class="mb-0">@foreach ($errors->all() as $error)<li>{{ $error }}</li>@endforeach</ul></div>@endif
                        <div class="row">
                            <div class="col-md-12">
                                <h5 class="mb-3 text-uppercase bg-light p-2 rounded"><i class="fas fa-info-circle me-2 text-primary"></i>Role Details</h5>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group mb-3">
                                    ...
                                    <div class="input-group">
                                        <span class="input-group-text"><i class="fas fa-user-shield"></i></span>
                                        <input type="text" class="form-control @error('role_name') is-invalid @enderror" name="role_name" id="edit_role_name" placeholder="Enter role name" value="{{ old('role_name') }}" required>
                                    </div>
                                    @error('role_name')
                                        <div class="invalid-feedback d-block">{{ $message }}</div>
                                    @enderror
                        ...
                        <div class="submit-section"><button type="submit" class="btn btn-primary submit-btn">Update</button></div>
                    </form>
                </div>
            </div>
        </div>
    ...
    <div id="delete_role" class="modal custom-modal fade" role="dialog">
        <div class="modal-dialog modal-dialog-centered"><div class="modal-content"><div class="modal-header"><h5 class="modal-title"><i class="fas fa-trash-alt me-2"></i>Delete Role</h5><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span>&times;</span></button></div><div class="modal-body text-center"><div class="mb-4"><i class="fas fa-exclamation-triangle text-warning" style="font-size:3rem"></i></div><h4 class="mb-3">Are you sure?</h4><p class="text-muted mb-4">This action cannot be undone. This will permanently delete the role.</p><div class="d-flex justify-content-center gap-3"><form id="deleteForm" method="POST" action="{{ route('form/role/delete') }}" class="d-inline">@csrf<input type="hidden" name="id" id="delete_id"><button type="submit" class="btn btn-action btn-action-delete me-3"><i class="fas fa-trash me-1"></i> Delete</button></form><button type="button" class="btn btn-secondary" data-dismiss="modal"><i class="fas fa-times me-1"></i> Cancel</button></div></div></div></div>
    </div>
    ...
    $(document).ready(function () {
        $(document).on('click', '.roleEdit', function (e) {
            ...
            $.ajax({
                ...
                error: function (xhr, status, error) {
                    ...
                    alert('Could not fetch role data.');
                }
            });
        });
        ...
        @if ($errors->any())
            @if (old('id'))
                ...
                setTimeout(function() {
                    $('#edit_id').val('{{ old('id') }}');
                    $('#edit_role_name').val('{{ old('role_name') }}');
