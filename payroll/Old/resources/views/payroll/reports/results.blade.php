            ...
            <div class="col-md-3">
                <div class="summary-card">
                    <div class="summary-card-icon" style="background: linear-gradient(135deg, #10b981, #059669);">
                        <i class="fa fa-money-bill-wave fa-lg text-white"></i>
                    </div>
                    <h6 class="summary-card-title">Total Gross Pay</h6>
                    <h4 class="summary-card-value"><span class="currency-symbol">{{ get_currency_symbol() }}</span>{{ number_format($totals['gross_pay'], 2) }}</h4>
                </div>
            </div>
            <div class="col-md-3">
                <div class="summary-card">
                    ...
                    <h4 class="summary-card-value"><span class="currency-symbol">{{ get_currency_symbol() }}</span>{{ number_format($totals['total_deduction'], 2) }}</h4>
                </div>
            </div>
            <div class="col-md-3">
                <div class="summary-card">
                    ...
                    <h4 class="summary-card-value"><span class="currency-symbol">{{ get_currency_symbol() }}</span>{{ number_format($totals['net_pay'], 2) }}</h4>
                </div>
            </div>
        </div>
                                                    ...
                                                    <tr>
                                                        ...
                                                        @foreach($earningComponents as $component)
                                                            <td class="text-center">
                                                                ...
                                                                @endphp
                                                                <span class="amount-positive"><span class="currency-symbol">{{ get_currency_symbol() }}</span>{{ number_format($value, 2) }}</span>
                                                            </td>
                                                        @endforeach
                                                        <td class="text-center"><span class="amount-positive"><span class="currency-symbol">{{ get_currency_symbol() }}</span>{{ number_format($attendance->gross_pay, 2) }}</span></td>
                                                        <td class="text-center"><span class="currency-symbol">{{ get_currency_symbol() }}</span>{{ number_format($attendance->epfWage ?? 0, 2) }}</span></td>
                                                        @foreach($deductionComponents as $component)
                                                            <td class="text-center">
                                                                @php
                                                                    $value = $attendance->deductions[$component->id]['value'] ?? 0;
                                                                @endphp
                                                                <span class="amount-negative"><span class="currency-symbol">{{ get_currency_symbol() }}</span>{{ number_format($value, 2) }}</span>
                                                            </td>
                                                        @endforeach
                                                        @if($hasAdvanceDeductions ?? false)
                                                            <td class="text-center">
                                                                ...
                                                                @endphp
                                                                <span class="amount-negative"><span class="currency-symbol">{{ get_currency_symbol() }}</span>{{ number_format($advanceValue, 2) }}</span>
                                                            </td>
                                                        @endif
                                                        <td class="text-center"><span class="amount-negative"><span class="currency-symbol">{{ get_currency_symbol() }}</span>{{ number_format($attendance->total_deduction, 2) }}</span></td>
                                                        <td class="text-center"><span class="font-weight-bold text-primary"><span class="currency-symbol">{{ get_currency_symbol() }}</span>{{ number_format($attendance->total_payable, 2) }}</span></td>
                                                    </tr>
                                                @endforeach
                                            </tbody>
                                            <tfoot>
                                                <tr class="font-weight-bold">
                                                    ...
                                                    @foreach($earningComponents as $component)
                                                        <td class="text-center">
                                                            ...
                                                            @endphp
                                                            <span class="amount-positive"><span class="currency-symbol">{{ get_currency_symbol() }}</span>{{ number_format($total, 2) }}</span>
                                                        </td>
                                                    @endforeach
                                                    <td class="text-center"><span class="amount-positive"><span class="currency-symbol">{{ get_currency_symbol() }}</span>{{ number_format($attendances->sum('gross_pay'), 2) }}</span></td>
                                                    <td class="text-center"><span class="currency-symbol">{{ get_currency_symbol() }}</span>{{ number_format($attendances->sum('epfWage'), 2) }}</span></td>
                                                    @foreach($deductionComponents as $component)
                                                        <td class="text-center">
                                                            @php
                                                                $total = 0;
                                                            ...
                                                            @endphp
                                                            <span class="amount-negative"><span class="currency-symbol">{{ get_currency_symbol() }}</span>{{ number_format($total, 2) }}</span>
                                                        </td>
                                                    @endforeach
                                                    <td class="text-center"><span class="amount-negative"><span class="currency-symbol">{{ get_currency_symbol() }}</span>{{ number_format($attendances->sum('total_deduction'), 2) }}</span></td>
                                                    <td class="text-center"><span class="font-weight-bold text-primary"><span class="currency-symbol">{{ get_currency_symbol() }}</span>{{ number_format($attendances->sum('total_payable'), 2) }}</span></td>
                                                </tr>
                                            </tfoot>
                                        </table>
                                    </div>
                                    ...
                                    <tbody>
                                        @foreach($consolidatedData as $empId => $data)
                                            <tr>
                                                ...
                                                @foreach($earningComponents as $component)
                                                    <td class="text-center">
                                                        <span class="amount-positive"><span class="currency-symbol">{{ get_currency_symbol() }}</span>{{ number_format($data['earnings'][(string)$component->id] ?? 0, 2) }}</span>
                                                    </td>
                                                @endforeach
                                                <td class="text-center"><span class="amount-positive"><span class="currency-symbol">{{ get_currency_symbol() }}</span>{{ number_format($data['gross_pay'], 2) }}</span></td>
                                                <td class="text-center"><span class="currency-symbol">{{ get_currency_symbol() }}</span>{{ number_format($data['epfWage'] ?? 0, 2) }}</span></td>
                                                @foreach($deductionComponents as $component)
                                                    <td class="text-center">
                                                        <span class="amount-negative"><span class="currency-symbol">{{ get_currency_symbol() }}</span>{{ number_format($data['deductions'][$component->id] ?? 0, 2) }}</span>
                                                    </td>
                                                @endforeach
                                                @if($hasAdvanceDeductions ?? false)
                                                    <td class="text-center">
                                                        <span class="amount-negative"><span class="currency-symbol">{{ get_currency_symbol() }}</span>{{ number_format($data['deductions']['advance'] ?? 0, 2) }}</span>
                                                    </td>
                                                @endif
                                                <td class="text-center"><span class="amount-negative"><span class="currency-symbol">{{ get_currency_symbol() }}</span>{{ number_format($data['total_deduction'], 2) }}</span></td>
                                                <td class="text-center"><span class="font-weight-bold text-primary"><span class="currency-symbol">{{ get_currency_symbol() }}</span>{{ number_format($data['total_payable'], 2) }}</span></td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                    <tfoot>
                                        <tr class="font-weight-bold">
                                            ...
                                            @endforeach
                                            <td class="text-center"><span class="amount-positive"><span class="currency-symbol">{{ get_currency_symbol() }}</span>{{ number_format($consolidatedData->sum('gross_pay'), 2) }}</span></td>
                                            <td class="text-center"><span class="currency-symbol">{{ get_currency_symbol() }}</span>{{ number_format($consolidatedData->sum('epfWage'), 2) }}</span></td>
                                            @foreach($deductionComponents as $component)
                                                <td class="text-center">
                                                    @php
                                                        $total = 0;
                                            ...
                                            @if($hasAdvanceDeductions ?? false)
                                                <td class="text-center">
                                                    ...
                                                    @endphp
                                                    <span class="amount-negative"><span class="currency-symbol">{{ get_currency_symbol() }}</span>{{ number_format($advanceTotal, 2) }}</span>
                                                </td>
                                            @endif
                                            <td class="text-center"><span class="amount-negative"><span class="currency-symbol">{{ get_currency_symbol() }}</span>{{ number_format($consolidatedData->sum('total_deduction'), 2) }}</span></td>
                                            <td class="text-center"><span class="font-weight-bold text-primary"><span class="currency-symbol">{{ get_currency_symbol() }}</span>{{ number_format($consolidatedData->sum('total_payable'), 2) }}</span></td>
                                        </tr>
                                    </tfoot>
                                </table>
                            </div>
