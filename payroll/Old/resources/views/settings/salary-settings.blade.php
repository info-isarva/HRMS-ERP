                        ...
                        <div class="settings-card">
                            <div class="card-header d-flex justify-content-between align-items-center">
                                ...
                                <div class="form-check form-switch custom-switch">
                                    ...
                                    <input class="form-check-input" type="checkbox" name="tds_enabled" id="switch_tds" value="1" {{ ($settings['salary_tds_enabled'] ?? '') == '1' ? 'checked' : '' }}>
                                </div>
                            </div>
                            <div class="card-body">
                                @php
                                    $slabsNew = json_decode($settings['salary_tds_slabs_new'] ?? '[]', true);
                                    if (empty($slabsNew)) {
                                        $slabsNew = [['from' => '', 'to' => '', 'percentage' => '']];
                                    }
                                    $slabsOld = json_decode($settings['salary_tds_slabs_old'] ?? '[]', true);
                                    if (empty($slabsOld)) {
                                        $slabsOld = [['from' => '', 'to' => '', 'percentage' => '']];
                                    }
                                @endphp

                                <ul class="nav nav-tabs mb-4" id="tdsRegimeTabs" role="tablist">
                                    <li class="nav-item">
                                        <a class="nav-link active fw-bold" id="new-regime-tab" data-toggle="tab" href="#new-regime" role="tab" aria-selected="true">
                                            <i class="fas fa-percentage me-1"></i> New Regime Slabs
                                        </a>
                                    </li>
                                    <li class="nav-item">
                                        <a class="nav-link fw-bold" id="old-regime-tab" data-toggle="tab" href="#old-regime" role="tab" aria-selected="false">
                                            <i class="fas fa-percentage me-1"></i> Old Regime Slabs
                                        </a>
                                    </li>
                                </ul>

                                <div class="tab-content" id="tdsRegimeTabsContent">
                                    <!-- New Regime Tab -->
                                    <div class="tab-pane fade show active" id="new-regime" role="tabpanel" aria-labelledby="new-regime-tab">
                                        <div id="tds_rows_container_new">
                                            @foreach($slabsNew as $index => $slab)
                                            <div class="row g-3 align-items-end mb-3 tds-row">
                                                <div class="col-md-4">
                                                    <div class="form-group mb-0">
                                                        <label class="form-label">Salary From (Annual)</label>
                                                        <input class="form-control" type="number" name="tds_slabs_new[{{ $index }}][from]" value="{{ $slab['from'] ?? '' }}" placeholder="From">
                                                    </div>
                                                </div>
                                                <div class="col-md-4">
                                                    <div class="form-group mb-0">
                                                        <label class="form-label">Salary To (Annual)</label>
                                                        <input class="form-control" type="number" name="tds_slabs_new[{{ $index }}][to]" value="{{ $slab['to'] ?? '' }}" placeholder="To (Leave empty for max)">
                                                    </div>
                                                </div>
                                                <div class="col-md-3">
                                                    <div class="form-group mb-0">
                                                        <label class="form-label">Tax Rate (%)</label>
                                                        <input class="form-control" type="number" step="0.01" name="tds_slabs_new[{{ $index }}][percentage]" value="{{ $slab['percentage'] ?? '' }}" placeholder="%">
                                                    </div>
                                                </div>
                                        ...
                                        <div class="row mt-3">
                                            <div class="col-md-2 ms-auto">
                                                <button class="btn btn-primary w-100" type="button" id="add_tds_row_btn_new"><i class="fa fa-plus"></i> Add Slab</button>
                                            </div>
                                        </div>
                                    </div>

                                    <!-- Old Regime Tab -->
                                    <div class="tab-pane fade" id="old-regime" role="tabpanel" aria-labelledby="old-regime-tab">
                                        <div id="tds_rows_container_old">
                                            @foreach($slabsOld as $index => $slab)
                                            <div class="row g-3 align-items-end mb-3 tds-row">
                                                <div class="col-md-4">
                                                    <div class="form-group mb-0">
                                                        <label class="form-label">Salary From (Annual)</label>
                                                        <input class="form-control" type="number" name="tds_slabs_old[{{ $index }}][from]" value="{{ $slab['from'] ?? '' }}" placeholder="From">
                                                    </div>
                                                </div>
                                                <div class="col-md-4">
                                                    <div class="form-group mb-0">
                                                        <label class="form-label">Salary To (Annual)</label>
                                                        <input class="form-control" type="number" name="tds_slabs_old[{{ $index }}][to]" value="{{ $slab['to'] ?? '' }}" placeholder="To (Leave empty for max)">
                                                    </div>
                                                </div>
                                                <div class="col-md-3">
                                                    <div class="form-group mb-0">
                                                        <label class="form-label">Tax Rate (%)</label>
                                                        <input class="form-control" type="number" step="0.01" name="tds_slabs_old[{{ $index }}][percentage]" value="{{ $slab['percentage'] ?? '' }}" placeholder="%">
                                                    </div>
                                                </div>
                                                <div class="col-md-1 text-center">
                                                    <button class="btn btn-danger btn-icon remove-tds-row d-flex align-items-center justify-content-center mx-auto" type="button" style="width: 42px; height: 42px; padding: 0; border-radius: 50%;"><i class="fas fa-trash-alt"></i></button>
                                        ...
                                        <div class="row mt-3">
                                            <div class="col-md-2 ms-auto">
                                                <button class="btn btn-primary w-100" type="button" id="add_tds_row_btn_old"><i class="fa fa-plus"></i> Add Slab</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        function setupTdsRows(containerId, addBtnId, inputNamePrefix) {
            ...
            if(addBtn && container) {
                addBtn.addEventListener('click', function() {
                    ...
                    if(rows.length > 0) {
                        ...
                        // Clear inputs and fix their names
                        const newIndex = container.querySelectorAll('.tds-row').length;
                        newRow.querySelectorAll('input').forEach(input => {
                            input.value = '';
                            const name = input.getAttribute('name');
                            if(name) {
                                // Replace index in name attribute, e.g. tds_slabs_new[0][from] -> tds_slabs_new[1][from]
                ...
                container.addEventListener('click', function(e) {
                    ...
                    if (deleteBtn) {
                        ...
                        if (rows.length > 1) {
                            ...
                            // Re-index names to prevent array holes on submit
                            container.querySelectorAll('.tds-row').forEach((row, rIdx) => {
                                row.querySelectorAll('input').forEach(input => {
                                    const name = input.getAttribute('name');
                                    if(name) {
                                        const updatedName = name.replace(/\[\d+\]/, '[' + rIdx + ']');
                                        input.setAttribute('name', updatedName);
                                    }
                                });
                            });
        ...
        setupTdsRows('tds_rows_container_new', 'add_tds_row_btn_new', 'tds_slabs_new');
        setupTdsRows('tds_rows_container_old', 'add_tds_row_btn_old', 'tds_slabs_old');
    });
</script>