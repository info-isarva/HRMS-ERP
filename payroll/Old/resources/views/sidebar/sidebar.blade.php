...
<aside id="sidebar" class="modern-sidebar fixed-top d-flex flex-column bg-white border-right">
    <div class="d-flex flex-column h-100">
        ...
        <nav class="flex-fill p-3 sidebar-nav">
            ...
            <!-- Compliance -->
            <div class="nav-section">
                <h6 class="nav-section-title text-muted text-uppercase font-weight-bold small mb-2">Compliance</h6>
                
                <a href="{{ route('compliance.data-requests') }}" class="nav-link {{ request()->routeIs('compliance.data-requests') ? 'active' : '' }}">
                    <i class="fas fa-shield-alt nav-icon text-success"></i>
                    <span class="nav-text">Privacy Requests</span>
                </a>

                @if(config('posh.legacy_enabled'))
                    @if(auth()->user()->role_name === 'Super Admin' || auth()->user()->role_name === 'Admin')
                        <a href="{{ route('compliance.posh.icc-board') }}" class="nav-link {{ request()->routeIs('compliance.posh.icc-board') ? 'active' : '' }}">
                            <i class="fas fa-users-cog nav-icon text-info"></i>
                            <span class="nav-text">POSH ICC Board <span class="badge badge-warning badge-sm ml-1" style="font-size:0.65rem;">Legacy</span></span>
                        </a>
                        <a href="{{ route('compliance.posh.policy-distribution') }}" class="nav-link {{ request()->routeIs('compliance.posh.policy-distribution') ? 'active' : '' }}">
                            <i class="fas fa-file-signature nav-icon text-primary"></i>
                            <span class="nav-text">POSH Auditing <span class="badge badge-warning badge-sm ml-1" style="font-size:0.65rem;">Legacy</span></span>
                        </a>
                    @endif
                    @if(auth()->user()->role_name === 'Super Admin' || auth()->user()->role_name === 'Admin' || \App\Models\PoshIccMember::where('employee_id', auth()->user()->employee_id)->exists())
                        <a href="{{ route('compliance.posh.complaints.index') }}" class="nav-link {{ request()->routeIs('compliance.posh.complaints.*') ? 'active' : '' }}">
                            <i class="fas fa-gavel nav-icon text-danger"></i>
                            <span class="nav-text">POSH Complaints <span class="badge badge-warning badge-sm ml-1" style="font-size:0.65rem;">Legacy</span></span>
                        </a>
                    @endif
                @else
                    <a href="{{ config('posh.workspace_url') }}/posh-access" class="nav-link" target="_blank" rel="noopener">
                        <i class="fas fa-shield-halved nav-icon" style="color:#1e3a5f;"></i>
                        <span class="nav-text">{{ config('posh.product_name') }}</span>
                    </a>
                @endif

                @if(auth()->user()->role_name === 'Super Admin' || auth()->user()->role_name === 'Admin')
                    <a href="{{ route('compliance.dpdp.policy-distribution') }}" class="nav-link {{ request()->routeIs('compliance.dpdp.policy-distribution') ? 'active' : '' }}">
                        <i class="fas fa-user-shield nav-icon text-warning"></i>
                        <span class="nav-text">DPDP Auditing</span>
                    </a>
                @endif
            </div>
