...
Route::group(['namespace' => 'App\Http\Controllers'],function()
{
    ...
    Route::controller(SettingController::class)->group(function () {
        Route::middleware('auth')->group(function () {
            ...
            Route::post('localization/save', 'saveLocalizationSettings')->name('localization/save'); /** save localization settings */
...
//Master Settings Only for Super Admin
    ...
    // --------------------------- Compliance & DPDP ---------------------------//
    Route::controller(\App\Http\Controllers\Compliance\DataChangeRequestController::class)->group(function () {
        Route::middleware('auth')->group(function () {
            Route::get('compliance/data-requests', 'index')->name('compliance.data-requests');
            Route::post('compliance/data-requests/{id}/resolve', 'resolve')->name('compliance.data-requests.resolve');
        });
    });
    ...
    Route::controller(EmployeeController::class)->group(function () {
        Route::middleware('auth')->group(function () {
         ...
         //   Route::get('employees/view', 'viewEmployee')->name('employees.view');
            ...
            Route::get('employees/masters-by-location', 'getMastersByLocation')->name('employees.masters-by-location');
    ...
    Route::controller(App\Http\Controllers\ComplianceController::class)->group(function () {
        ...
        Route::middleware('auth')->group(function () {
            Route::get('compliance/dpdp-dashboard', 'dpdpDashboard')->name('compliance.dpdp.dashboard');
            Route::post('compliance/dpdp-export', 'requestDataExport')->name('compliance.dpdp.export');
            Route::get('compliance/dpdp/policy-distribution', 'policyDistribution')->name('compliance.dpdp.policy-distribution');
        });
    });

    // POSH Act Compliance Admin Routes (deprecated — Phase 0 redirects when POSH_LEGACY_ENABLED=false)
    Route::middleware(['auth', 'posh.legacy.block'])->controller(\App\Http\Controllers\Compliance\PoshComplianceController::class)->group(function () {
        Route::get('compliance/posh/icc-board', 'iccBoard')->name('compliance.posh.icc-board');
        Route::post('compliance/posh/icc-board/store', 'storeIccMember')->name('compliance.posh.icc-board.store');
        Route::post('compliance/posh/icc-board/{id}/update', 'updateIccMember')->name('compliance.posh.icc-board.update');
        Route::delete('compliance/posh/icc-board/{id}/delete', 'deleteIccMember')->name('compliance.posh.icc-board.delete');
        
        Route::get('compliance/posh/policy-distribution', 'policyDistribution')->name('compliance.posh.policy-distribution');
        
        Route::get('compliance/posh/complaints', 'complaints')->name('compliance.posh.complaints.index');
        Route::get('compliance/posh/complaints/{id}', 'complaintDetails')->name('compliance.posh.complaints.show');
        Route::post('compliance/posh/complaints/{id}/log', 'logComplaintAction')->name('compliance.posh.complaints.log');
    });
