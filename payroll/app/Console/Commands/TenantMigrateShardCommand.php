...
class TenantMigrateShardCommand extends Command
{
    protected $signature = 'tenant:migrate-shard
        {database : Payroll shard database name e.g. hrms_company_b_payroll}
        {--create : Create database if missing (requires MySQL CREATE privilege)}';

    protected $description = 'Run payroll migrations on a tenant shard database (Company B provisioning)';
    ...
    public function handle(): int
    {
        ...
        $this->info("Running payroll migrations on [{$database}]...");
        ...
        $this->info("Shard [{$database}] is ready. Register tenant in hrms_central.");
