...
class EmployeeController extends Controller
{
    public function index()
    {
        ...
        $employees = EmployeeBasicDetail::with([
            ...
            ->get()
            ->map(function ($employee) use ($designations, $roles, $departments, $employeeStatuses, $currentFYName) {
                return [
                    'id' => $employee->id,
                    ...
                    'leave_allocations' => $employee->leaveAllocations ? $employee->leaveAllocations->map(function($allocation) {
                        return [
                            'id' => $allocation->id,
                    ...
                    'leave_summary' => [
                        ...
                        'total_allocated_days' => $employee->leaveAllocations ? $employee->leaveAllocations->sum('effective_days') : 0,
                        'total_leave_types' => $employee->leaveAllocations ? $employee->leaveAllocations->count() : 0,
                        'has_overrides' => $employee->leaveAllocations ? $employee->leaveAllocations->where('is_manual_override', true)->count() > 0 : false,
                    ],
        ...
        return response()->json([
            ...
            'data' => [
                ...
                'leave_summary' => [
                    ...
                    'total_leave_types' => $employee->leaveAllocations ? $employee->leaveAllocations->count() : 0,
                    'has_overrides' => $employee->leaveAllocations ? $employee->leaveAllocations->where('is_manual_override', true)->count() > 0 : false,
                    'pro_rated_allocations' => $employee->leaveAllocations ? $employee->leaveAllocations->where('is_pro_rated', true)->count() : 0,
                ],
                'week_off_configuration' => $employee->weekOff ? $employee->weekOff->toApiArray() : [
                    'week_off_days' => [0], // Default Sunday
                    'week_off_pattern' => 'Sunday',
