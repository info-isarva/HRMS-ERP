                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                ...
                                <h4 class="mb-0">₹<span id="total-incentive-amount">{{ number_format($totalIncentiveAmount, 0) }}</span></h4>
                            </div>
                        ...
                        <tbody>
                            @foreach($employees as $employee)
                                ...
                                <tr data-employee-id="{{ $employee->id }}" data-employee-tag-id="{{ $employee->employee_id }}">
                                    ...
                                    <td>{{ $employee->name }}</td>
                                    {{-- <td>₹{{ number_format($employee->incentive_per_month, 2) }}</td> --}}
                                    <td>₹{{ number_format($employee->incentive_per_month, 2) }}</td>

                                    <td>₹{{ number_format($dailyRate) }}</td>
                                    <td>
                                        <input type="number" 
    ...
    const apiToken = '{{ env("ATTENDANCE_API_TOKEN") }}';
        ...
        document.querySelectorAll('.incentive-days').forEach(input => {
            input.addEventListener('input', function() {
                ...
                // this.closest('tr').querySelector('.incentive-total').textContent = '₹' + Math.round(total);
                // const dailyRate = Math.ceil(monthly / {{ $totalDays }});
                // const total = dailyRate * days;
                // this.closest('tr').querySelector('.incentive-total').textContent = '₹' + total.toFixed(2);
                const dailyRate = Math.ceil(monthly / {{ $totalDays }});
