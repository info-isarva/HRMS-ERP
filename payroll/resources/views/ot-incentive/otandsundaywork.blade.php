                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="alert alert-warning mb-0">
                            ...
                            Please click "Fetch Data" to retrieve overtime and holiday work hours before saving or finalizing.
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-center">
                                <div>
                                    ...
                                    <h3 class="metric-value text-success">₹<span id="total-ot-amount">{{ number_format($totalOtAmount, 0) }}</span></h3>
                                </div>
                        ...
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-center">
                                <div>
                                    ...
                                    <h3 class="metric-value text-info">₹<span id="total-holiday-amount">{{ number_format($totalHolidayAmount, 0) }}</span></h3>
                                </div>
                                <div class="metric-icon">
                        ...
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-center">
                                <div>
                                    ...
                                    <h3 class="metric-value text-warning">₹<span id="grand-total">{{ number_format($totalOtAmount + $totalHolidayAmount, 0) }}</span></h3>
                                </div>
                                <div class="metric-icon">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('ot-incentive.save-ot-holiday', [$month, $year]) }}" id="ot-form">
                        ...
                        <div class="tab-content" id="otHolidayTabContent">
                                            ...
                                            @foreach($otEmployees as $employee)
                                                ...
                                                @if(!$isFinalized || ($isFinalized && $otHours > 0))
                                                    <tr data-employee-id="{{ $employee->id }}" data-employee-tag-id="{{ $employee->employee_id }}">
                                                        ...
                                                        <td>{{ $employee->name }}</td>
                                                        <td>₹{{ number_format($employee->ot_per_hour, 2) }}</td>
                                                        <td>
                                                            <input type="number"
                                                        ...
                                                        <td class="ot-total">
                                                            ₹{{ number_format(($existing->total_amount ?? 0), 2) }}
                                                        </td>
                                                    </tr>
                                                @endif
                                            @endforeach
                            ...
                            <!-- Holiday Tab -->
                            <div class="tab-pane fade" id="holiday-pane" role="tabpanel" aria-labelledby="holiday-tab">
                                <div class="table-responsive">
                                    <table class="table table-striped custom-table">
                                        <thead>
                                            <tr>
                                            ...
                                            @foreach($holidayEmployees as $employee)
                                                ...
                                                @if(!$isFinalized || ($isFinalized && $holidayDays > 0))
                                                    <tr data-employee-id="{{ $employee->id }}" data-daily-rate="{{ $employee->daily_rate }}">
                                                        <td>{{ $employee->employee_id }}</td>
                                                        <td>{{ $employee->name }}</td>
                                                        <td>₹{{ number_format($employee->total_earnings, 2) }}</td>
                                                        <td>
                                                            <input type="number"
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="alert alert-warning mb-3">
                            <i class="fas fa-lock me-2"></i> OT and Holiday work for this month has been finalized and cannot be edited.
                        </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Are you sure you want to save and finalize OT and Holiday work for this month?</p>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (fetchButton) {
        fetchButton.addEventListener('click', function() {
            ...
            Promise.all([
                ...
                fetch(`${window.ATTENDANCE_API_BASE_URL}/attendance-data?month={{ $month }}&year={{ $year }}`)
            ])
            .then(responses => Promise.all(responses.map(res => res.json())))
            .then(([otData, holidayData]) => {
                let successCount = 0;
                let errorMessages = [];

                // Process OT data
                if (otData.status === 'success' && otData.data && otData.data.length > 0) {
                    otData.data.forEach(record => {
                        const rows = document.querySelectorAll('#ot-pane tr[data-employee-tag-id]');
                        rows.forEach(row => {
                            const empTagId = row.getAttribute('data-employee-tag-id');
                            if (empTagId === record.employee_tag_id) {
                                const otInput = row.querySelector('.ot-hours');
                                if (otInput) {
                                    otInput.value = parseFloat(record.overtime_hr);
                ...
                if (holidayData.status === 'success' && holidayData.data && holidayData.data.length > 0) {
                    holidayData.data.forEach(record => {
                        const rows = document.querySelectorAll('#holiday-pane tr[data-employee-id]');
                        rows.forEach(row => {
                            const empActId = row.getAttribute('data-employee-id');
                            // Ensure both IDs are strings for comparison
                            if (empActId === String(record.emp_id) && parseFloat(record.payoff_days) > 0) {
                                const daysInput = row.querySelector('.holiday-days');
                                if (daysInput) {
                                    daysInput.value = parseFloat(record.payoff_days);
                                    const event = new Event('input', { bubbles: true });
                    ...
                    errorMessages.push('Holiday: ' + (holidayData.message || 'No data found'));
                }
    ...
    document.querySelectorAll('.ot-hours').forEach(input => {
        input.addEventListener('input', function() {
            ...
            this.closest('tr').querySelector('.ot-total').textContent = '₹' + total.toFixed(2);
            updateTotals();
        });
    });
    ...
    document.querySelectorAll('.holiday-days').forEach(input => {
        input.addEventListener('input', function() {
            ...
            row.querySelector('.holiday-rate').textContent = '₹' + dailyRate.toFixed(2);
            row.querySelector('.holiday-total').textContent = '₹' + total.toFixed(2);
            updateTotals();
        });
    });
    ...
    function updateTotals() {
        ...
        document.querySelectorAll('.ot-total').forEach(cell => {
            totalOt += parseFloat(cell.textContent.replace(/[₹,]/g, '')) || 0;
        });

        let totalHoliday = 0;
        document.querySelectorAll('.holiday-total').forEach(cell => {
            totalHoliday += parseFloat(cell.textContent.replace(/[₹,]/g, '')) || 0;
        });

        document.getElementById('total-ot-amount').textContent = totalOt.toLocaleString('en-IN');
        document.getElementById('total-holiday-amount').textContent = totalHoliday.toLocaleString('en-IN');
        document.getElementById('grand-total').textContent = (totalOt + totalHoliday).toLocaleString('en-IN');
    }
});
</script>
