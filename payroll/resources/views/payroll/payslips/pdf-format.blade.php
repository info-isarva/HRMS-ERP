...
@endphp
    ...
    <div class="payslip-card">
                    ...
                    <tbody>
                      ...
                      @for($i = 0; $i < $maxRows; $i++)
                        <tr>
                            ...
                            <td>{{ $applicableEarnings->get($i) ? '₹ ' . round($applicableEarnings->get($i)['value']) : '' }}</td>
                            <td>{{ $applicableDeductions->get($i)['name'] ?? '' }}</td>
                            <td>{{ $applicableDeductions->get($i) ? '₹ ' . round($applicableDeductions->get($i)['value']) : '' }}</td>
                        </tr>
                      @endfor
                      ...
                      @if($maxRows > 0)
                        <tr class="total-row">
                            ...
                            <td><strong>₹{{ number_format($attendances[0]->totalEarnings, 0) }}/-</strong></td>
                            <td><strong>Total Deductions</strong></td>
                            <td><strong>₹{{ number_format($attendances[0]->totalDeductions, 0) }}/-</strong></td>
                        </tr>
                      @endif
                    </tbody>
                </table>
                ...
                <div class="net-pay">
                    <strong>Net Pay: ₹{{ number_format($attendances[0]->netPay, 0) }}/-</strong><br>
                    <em>{{ \App\Helper\NumberHelper::numberToWordsIndian($attendances[0]->netPay) }} Rupees Only</em>
                </div>
