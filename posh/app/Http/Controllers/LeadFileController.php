...
class LeadFileController extends Controller
{
    ...
    {
        // Prevent creating/editing/deleting lead files when a historical financial year is selected.
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'file_link' => 'nullable|string|max:2000', // allow multiple links as text
        ...
        if ($request->file_type === 'file upload') {
            if ($request->hasFile('file_upload')) {
                ...
                $leadFolder = 'lead_files/' . $request->related_id;
                $filename = $originalName . '_' . time() . '.' . $originalExtension;

                //         $folder = 'avatars/' . $user->id;
                // $filename = $originalName . '_' . time() . '.' . $originalExtension;
                $relativeDir = 'assets/leads/' . $leadFolder;
            ...
            if (!empty($request->file_link)) {
                // Split by newlines or comma, trim, and filter valid URLs
                $links = preg_split('/[\n,]+/', $request->file_link);
                $count = 0;
