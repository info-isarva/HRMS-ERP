...
<link rel="stylesheet" href="{{ asset('css/deals-edit-custom.css') }}">
    <div class="container-fluid p-4">
        <div class="card mt-0">
            <form id="dealCreateForm" method="POST" action="{{ route('deals.store') }}">
                @csrf
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h4 class="mb-0">Create Deal</h4>
                    <div class="text-end">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="title" name="title" value="{{ old('title') }}"
                                   >
                                <div id="deal_title_error" class="text-danger small mt-1" style="display:none;"></div>
                                @error('title')
                                    <div class="text-danger small mt-1">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="lead_source" name="lead_source" >
                                    ...
                                    @foreach($leadSources as $source)
                                        <option value="{{ $source->id }}" @if(old('lead_source') == $source->id) selected @endif>{{ $source->name }}</option>
                                    @endforeach
                                </select>
                                @error('lead_source')
                                    <div class="text-danger small mt-1">{{ $message }}</div>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="label" name="label" >
                                    ...
                                    <option value="normal" @if(old('label')=='normal') selected @endif>Normal</option>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="user_owner_id" name="user_owner_id" >
                                    ...
                                    @foreach($users as $user)
                                        <option value="{{ $user->id }}" @if(old('user_owner_id', isset($currentUser) ? $currentUser->id : null) == $user->id) selected @endif >{{ $user->name }}</option>
                                    @endforeach
                                </select>
                                @error('user_owner_id')
                                    <div class="text-danger small mt-1">{{ $message }}</div>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="stage" name="stage" >
                                    ...
                                    @foreach($stages as $stage)
                                        <option value="{{ $stage->name }}" data-probability="{{ $stage->probability ?? 10 }}" @if(old('stage') == $stage->name) selected @endif>{{ $stage->name }} ({{ $stage->probability ?? 'N/A' }}%)</option>
                                    @endforeach
                                </select>
                                @error('stage')
                                    <div class="text-danger small mt-1">{{ $message }}</div>
                            ...
                            <div class="mb-3" id="reason_for_loss_container" style="display: {{ old('stage') == 'Closed Lost' ? 'block' : 'none' }};">
                             ...
                             <div class="mb-3">
                                ...
                                <input type="number" class="form-control" id="probability" name="probability" min="0" max="100" value="{{ old('probability', 10) }}" oninput="if(this.value.length>3)this.value=this.value.slice(0,3);">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" placeholder="+91 1234567890" class="form-control" id="org_phone" name="org_phone">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" placeholder="https://example.com" class="form-control" id="org_website" name="org_website">
...
<!-- <script src="{{asset('js/deals/create-deals.js')}}"></script> -->
 <script>
document.addEventListener('DOMContentLoaded', function() {
         const CSRF_TOKEN = (document.querySelector('meta[name="csrf-token"]') && document.querySelector('meta[name="csrf-token"]').getAttribute('content')) || '';
            ...
            // Show/hide reason_for_loss in create form and mark required when Closed Lost
            var createStage = document.getElementById('stage');
            var createReasonContainer = document.getElementById('reason_for_loss_container');
            var createReason = document.getElementById('reason_for_loss');
            function handleCreateStageChange() {
                if (!createStage || !createReasonContainer) return;
                // Toggle reason_for_loss visibility
                if (createStage.value === 'Closed Lost') {
                    createReasonContainer.style.display = 'block';
                    // if (createReason) createReason.setAttribute('required', 'required');
                } else {
                    createReasonContainer.style.display = 'none';
                    if (createReason) { createReason.removeAttribute('required'); createReason.value = ''; }
                }
                // Update probability input based on selected stage's data-probability
                var probabilityInput = document.getElementById('probability');
                if (probabilityInput && createStage.options && createStage.selectedIndex >= 0) {
                    var selectedOpt = createStage.options[createStage.selectedIndex];
                    var prob = selectedOpt ? selectedOpt.getAttribute('data-probability') : null;
                    if (prob !== null && prob !== '') {
                        // only update if not manually edited by user recently – but for now set value
                        probabilityInput.value = prob;
                    }
                }
            }
            if (createStage) {
                createStage.addEventListener('change', handleCreateStageChange);
                // initial
...
//         }
//     });
//     // New contact person modal submit with duplicate check and single word validation
//     document.getElementById('personNewForm').addEventListener('submit', function (e) {
//         e.preventDefault();
//         // Hide all error messages first
...
//         }
//         if (errorMsg) {
//             const fnErr = document.getElementById('person_first_name_error');
//             if (fnErr) {
//                 fnErr.textContent = errorMsg;
//                 fnErr.style.display = 'block';
//             }
//             return;
//         }

//         // Prepare fullname and organization context for duplicate check
//         const fullName = (firstName + (lastName ? ' ' + lastName : '')).trim();
//         const orgSelect = document.getElementById('person_org_id');
//         const orgName = orgSelect && orgSelect.options[orgSelect.selectedIndex] ? orgSelect.options[orgSelect.selectedIndex].text : (document.getElementById('organization_id').value || '');

//         // client-side pre-check for duplicates within the selected organization
//         if (fullName) {
//             const q = encodeURIComponent(fullName);
//             const checkUrl = '/people/autocomplete?q=' + q + (orgName ? '&organization=' + encodeURIComponent(orgName) : '');
//             fetch(checkUrl, { headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' }, credentials: 'same-origin' })
//                 .then(res => res.json())
//                 .then(list => {
//                     const found = (list || []).find(p => {
//                         const candidate = ((p.first_name || '') + ' ' + (p.last_name || '')).trim();
//                         return candidate.toLowerCase() === fullName.toLowerCase();
//                     });
//                     if (found) {
//                         const err = document.getElementById('person_first_name_error');
//                         if (err) {
//                             err.textContent = 'A contact with this name already exists for the selected company.';
//                             err.style.display = 'block';
//                         } else {
//                             alert('A contact with this name already exists for the selected company.');
//                         }
//                         const fn = document.getElementById('person_first_name'); if (fn) fn.focus();
//                         return;
//                     }

//                     // Not found, proceed to create via AJAX
//                     const formData = new FormData(document.getElementById('personNewForm'));
//                     // append CSRF token if available (static JS may be cached)
...
//                         })
//                         .then(person => {
//                             // server may respond with duplicate payload
//                             if (person && person.duplicate) {
//                                 const err = document.getElementById('person_first_name_error');
//                                 if (err) {
//                                     err.textContent = 'A contact with this name already exists for the selected company.';
...
//                             }
//                             personInput.value = (person.first_name ? person.first_name : '') + (person.last_name ? ' ' + person.last_name : '');
//                             try { var modal = bootstrap.Modal.getInstance(document.getElementById('personNewModal')); if (modal) modal.hide(); } catch (e) {}
//                         })
//                         .catch(() => {});
//                 })
//                 .catch(() => {
//                     // On error during duplicate check, fall back to direct create
//                     const formData = new FormData(document.getElementById('personNewForm'));
//                     const CSRF_TOKEN = (document.querySelector('meta[name="csrf-token"]') || {}).content || '';
//                     if (CSRF_TOKEN) formData.append('_token', CSRF_TOKEN);
//                     fetch('/people/ajax-create', {
...
//             return;
//         }
//         // fallback — shouldn't reach here because fullName required, but keep compatibility
//     });
...
//     updateProbability(); // Initial fill
// });
// // Prevent duplicate deals: check title + organization + contact person before submitting
// document.addEventListener('DOMContentLoaded', function () {
//     const dealForm = document.getElementById('dealCreateForm');
//     if (!dealForm) return;
//     dealForm.addEventListener('submit', function (e) {
//         e.preventDefault();
//         const errDiv = document.getElementById('deal_title_error');
//         if (errDiv) { errDiv.style.display = 'none'; errDiv.textContent = ''; }
//         const title = (document.getElementById('title').value || '').trim();
//         const org = (document.getElementById('organization_id').value || '').trim();
//         const person = (document.getElementById('people_id').value || '').trim();

//         // If any of the three key fields is missing, let server-side validation handle it
//         if (!title || !org || !person) {
//             dealForm.submit();
//             return;
//         }

//         const checkUrl = '/deals/check-duplicate?title=' + encodeURIComponent(title) + '&organization=' + encodeURIComponent(org) + '&person=' + encodeURIComponent(person);
//         fetch(checkUrl, { headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' }, credentials: 'same-origin' })
//             .then(res => res.json())
//             .then(data => {
//                 if (data && data.duplicate) {
//                     const err = document.getElementById('deal_title_error');
//                     if (err) {
//                         err.textContent = 'A deal with the same title, company and contact person already exists.';
//                         err.style.display = 'block';
//                         document.getElementById('title').focus();
//                     } else {
//                         alert('A deal with the same title, company and contact person already exists.');
//                     }
//                     return;
//                 }
//                 // No duplicate found — submit normally
//                 dealForm.submit();
//             })
//             .catch(() => {
//                 // On error while checking, allow submission (server will authoritative validate)
//                 dealForm.submit();
//             });
//     });
// });
