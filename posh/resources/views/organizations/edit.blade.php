...
<link rel="stylesheet" href="{{ asset('css/organization-custom.css') }}">
    <div class="container-fluid p-4">
        <!-- @if (session('success'))
            <div id="orgSuccessMsg"
                class="alert alert-success fade show position-fixed top-0 start-50 translate-middle-x mt-3"
                style="z-index: 2000; min-width: 300px; text-align: center; transition: opacity 0.5s;">
                {{ session('success') }}
            </div>
        @endif -->
        <form method="POST" action="{{ route('organizations.update', $organization->id) }}" id="orgEditForm" novalidate>
            @csrf
            @method('PUT')
            <div class="card mt-0">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <div class="row mb-3">
                                    ...
                                    <select class="form-select" id="industry_type" name="industry_type">
                                        ...
                                        @foreach ($industries as $industry)
                                            <option value="{{ $industry->id }}"
                                                @if ($industry->id == old('industry_type', $organization->industry_type)) selected @endif>{{ $industry->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    @error('industry_type')
                                ...
                                <div class="col-6">
                                    <label for="organization_type" class="form-label">Type <span
                                            class="text-danger">*</span></label>
                                    <select class="form-select" id="organization_type" name="organization_type">
                                        <option value="">Select type</option>
                                        @foreach ($organizationTypes as $typeId => $typeName)
                                            <option value="{{ $typeId }}"
                                                @if ($typeId == old('organization_type', $organization->organization_type)) selected @endif>{{ $typeName }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="url" placeholder="https://example.com" class="form-control" id="website"
                                    name="website" value="{{ old('website', $organization->website) }}">
                                @error('website')
                                    <div class="text-danger small">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" id="description" name="description" rows="2">{{ old('description', $organization->description) }}</textarea>
                                @error('description')
                                    <div class="text-danger small">{{ $message }}</div>
                                @enderror
                            </div>
                                    ...
                                    <select class="form-select" id="user_owner_id" name="user_owner_id">
                                        ...
                                        @foreach ($owners as $owner)
                                            <option value="{{ $owner->id }}"
                                                @if ($owner->id == old('user_owner_id', $organization->user_owner_id)) selected @endif>{{ $owner->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    @error('user_owner_id')
                        ...
                        <div class="col-md-6">
                                    ...
                                    <select class="form-select" id="country" name="country">
                                        ...
                                        @foreach ($countries as $country)
                                            <option value="{{ $country }}"
                                                @if ($country == old('country', $organization->country)) selected @endif>{{ $country }}
                                            </option>
                                        @endforeach
                                    </select>
                                    @error('country')
                            ...
                            <div class="mb-3">
                                ...
                                <input type="tel" placeholder="+91 98765 43210" class="form-control" id="phone"
...
<script>
    (function(){
        ...
        document.addEventListener('DOMContentLoaded', function(){
            ...
            var initialCountry = countryEl ? countryEl.value || '{{ old("country", $organization->country ?? "India") }}' : '{{ old("country", $organization->country ?? "India") }}';
